...
const lables = students.map(student => `${student.name} (${student.age})`);
...
const passed17aged = students.filter(student => student.grade <= 4).filter(student => student.age === 17).join(', ') 
const passed17aged = students.filter(student => student.grade <= 4 && student.age === 17).join(', ') 
